...
//
// Workflow canvas page — drag-and-drop nodes, connect with arrows,
// save to / load from the Rust + PostgreSQL backend.
    ...
    <g onClick={onClick} style={{ cursor: "pointer" }}>
      ...
      {selected && (
        <g transform={`translate(${mx},${my})`}
          ...
          <text x={0} y={4} textAnchor="middle" fill="white" fontSize={13} fontWeight={700}
...
const WorkflowNodeShape: FC<WorkflowNodeProps> = ({
  node, selected, connecting, onMouseDown, onStartConnect, onEndConnect, onSelect, onLabelChange,
}) => {
    ...
    >
      {isDecision ? (
        <polygon
          ...
          fill={def.color + "22"} stroke={selected ? def.color : def.color + "88"}
          strokeWidth={selected ? 2.5 : 1.5}
        />
      ) : (
        <rect width={NODE_W} height={NODE_H}
          rx={node.type === "start" || node.type === "end" ? 30 : 10}
          fill={def.color + "18"} stroke={selected ? def.color : def.color + "66"}
          strokeWidth={selected ? 2.5 : 1.5}
        />
      )}
      {/* ID badge */}
      <text x={NODE_W - 6} y={12} textAnchor="end" fill={def.color} fontSize={9}
        fontWeight={600} opacity={0.7} style={{ userSelect: "none", fontFamily: "monospace" }}>
        {node.id}
      </text>
      {/* Icon */}
      <text x={20} y={NODE_H / 2 + 5} textAnchor="middle" fill={def.color} fontSize={16}
        style={{ userSelect: "none" }}>{def.icon}</text>
      {/* Label */}
      {editing ? (
        ...
        <text x={NODE_W / 2 + 6} y={NODE_H / 2 + 4} textAnchor="middle"
      ...
      {/* Connect port */}
      <circle cx={NODE_W / 2} cy={NODE_H} r={7}
        fill={connecting ? def.color + "88" : "#1e293b"} stroke={def.color} strokeWidth={1.5}
      ...
      />
      <text x={NODE_W / 2} y={NODE_H + 4} textAnchor="middle" fill={def.color}
        fontSize={8} style={{ userSelect: "none", pointerEvents: "none" }}>+</text>
    </g>
  );
};
...
const WorkflowDesigner: FC = () => {
  ...
  const handleSaveConfirm = async (name: string, description: string) => {
    ...
    try {
      let id = workflowId;
      // First save → create the workflow row
      if (!id) {
      ...
      showToast("Save failed — check the server.", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleLoad = async (id: string) => {
    ...
    try {
      ...
      // Rebuild local canvas state from the API response
      ...
      const loadedEdges: WorkflowEdge[] = wf.edges.map(e => ({
        id: `e${e.id}`,
        from: e.from_node_canvas_id,
        to: e.to_node_canvas_id,
      }));
      ...
      // Bump the local id counter past any existing canvas ids
  ...
  return (
    <div style={{ display: "flex", height: "calc(100vh - 64px)", background: "#0f172a",
      fontFamily: "system-ui, sans-serif", color: "#e2e8f0", position: "relative" }}>

      {/* ── Toast ──────────────────────────────────────────────────────────── */}
      {toast && (
        <div style={{
          position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)",
          zIndex: 100, padding: "10px 20px", borderRadius: 8, fontSize: 13,
      ...
      <aside style={{ width: 220, flexShrink: 0, borderRight: "1px solid #1e293b",
        ...
        <div style={{ padding: "20px 16px 12px", borderBottom: "1px solid #1e293b" }}>
          <p style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase",
            color: "#64748b", margin: "0 0 4px" }}>Workflow Designer</p>
          {workflowId && (
        ...
        <div style={{ padding: "12px 16px", borderBottom: "1px solid #1e293b" }}>
          <p style={{ fontSize: 11, letterSpacing: "0.08em", color: "#475569",
            margin: "0 0 8px", textTransform: "uppercase" }}>Nodes</p>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {(["x", "y"] as const).map(axis => (
                    <div key={axis} style={{ flex: 1 }}>
                      ...
                      <input type="number" value={Math.round(selectedNode[axis])}
                        onChange={e => setNodes(ns => ns.map(n =>
      ...
      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}
        onDrop={handleDrop} onDragOver={(e: DragEvent<HTMLDivElement>) => e.preventDefault()}>
        ...
        <div style={{ position: "absolute", top: 12, left: 16, zIndex: 10,
          ...
          {/* Load */}
          <button onClick={() => setLoadPanelOpen(true)} disabled={loading}
            style={{ padding: "6px 14px", borderRadius: 8, fontSize: 12,
              cursor: loading ? "not-allowed" : "pointer",
              background: loading ? "#1e293b" : "#8b5cf622",
              border: "1px solid #8b5cf666", color: "#c4b5fd", opacity: loading ? 0.7 : 1 }}>
          ...
          <button onClick={() => {
            ...
            setWorkflowId(null); setWorkflowName("Untitled workflow"); setWorkflowDesc("");
          }} style={{ padding: "6px 14px", borderRadius: 8, fontSize: 12, cursor: "pointer",
        ...
        <svg ref={svgRef} width="100%" height="100%"
          ...
          <defs>
            ...
            <marker id="arrow-sel" viewBox="0 0 10 10" refX={8} refY={5}
              markerWidth={6} markerHeight={6} orient="auto-start-reverse">
              <path d="M2 1L8 5L2 9" fill="none" stroke="#3b82f6" strokeWidth={1.5}
                strokeLinecap="round" strokeLinejoin="round"/>
            </marker>
            <pattern id="grid" width={40} height={40} patternUnits="userSpaceOnUse"
              x={pan.x % 40} y={pan.y % 40}>
          ...
          <g transform={`translate(${pan.x},${pan.y})`}>
            {edges.map(edge => (
              <Arrow key={edge.id} from={edge.from} to={edge.to} nodes={nodes}
