// src/components/designer/WorkflowListModal.tsx
// Slide-in panel listing all saved MBRs so the supervisor can load one into
// the designer canvas.
...
const WorkflowListModal: FC<WorkflowListModalProps> = ({ open, onClose, onLoad }) => {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this MBR permanently?')) return;
    setDeletingId(id);
    try {
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          ...
          <div>
            <div style={{ fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase',
              color: '#475569', marginBottom: 2 }}>MBR Library</div>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#e2e8f0' }}>Saved Workflows</div>
          </div>
          <button
            onClick={onClose}
          ...
          {workflows.map((wf) => (
            <div
              ...
              style={{ padding: '12px 14px', borderRadius: 8,
                border: '1px solid #1e293b', marginBottom: 8, background: '#0a0f1c' }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, margin: 0, color: '#e2e8f0',
                ...
                <div style={{ display: 'flex', gap: 6, marginLeft: 10, flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    style={{ padding: '5px 10px', borderRadius: 6, fontSize: 11,
                      ...
                      opacity: deletingId === wf.id ? 0.5 : 1, fontFamily: 'inherit' }}
                  >
